                ...
                {chartData.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: -20, bottom: 0 }}>
                            ...
                            <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 8px 30px rgba(0,0,0,0.05)', padding: '12px', backgroundColor: 'rgba(255, 255, 255, 0.95)' }} labelStyle={{ fontWeight: 'bold', color: '#1e293b' }} />
                            {threshold && (
                                <ReferenceLine y={threshold.value} stroke={threshold.color} strokeDasharray="5 5"
                                    label={{ value: threshold.label, fill: threshold.color, fontSize: 10, fontWeight: 'bold', position: 'top' }} />
                            )}
                            ...
                            <Line type="monotone" dataKey="historyValue" stroke={mainColor} strokeWidth={3} dot={{ r: 3, fill: mainColor, strokeWidth: 1.5, stroke: '#fff' }} activeDot={{ r: 5, strokeWidth: 0 }} name={`${activeTab.toUpperCase()} (Aktual)`} connectNulls={false} />
