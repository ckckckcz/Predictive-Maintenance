...
export default function LinePage() {
	...
	const handleCreate = async () => {
		...
		try {
			await lineService.create(name, code)
	...
	const handleUpdate = async () => {
		...
		try {
			await lineService.update(selectedLine.id, name, code)
		...
		<div className="space-y-6">
			<div className="space-y-4">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div className="space-y-1">
						<h1 className="text-2xl font-bold text-gray-900 tracking-tight">Manajemen Line Produksi</h1>
						<p className="text-gray-500 text-sm">Kelola data line produksi dan alur pemrosesan di pabrik PT. Greenfields Indonesia.</p>
					</div>
					...
					<div className="flex items-center gap-2">
						...
						{isSupervisor && (
							<Button onClick={() => { setName(""); setCode(""); setIsFormOpen(true) }} className="gap-2 text-white h-11 bg-emerald-600 hover:bg-emerald-700 font-semibold cursor-pointer shadow-md shadow-emerald-100">
								<Plus className="h-4 w-4" /> Tambah Line
							</Button>
						)}
					</div>
				</div>
				...
				<div className="relative w-full">
					...
					<Input placeholder="Cari line berdasarkan nama atau kode..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-11 h-12 bg-white border-gray-200 focus-visible:ring-emerald-500 focus-visible:ring-1" />
				</div>
			</div>
			...
			>
				<div className="space-y-4 py-2">
					<div className="space-y-1.5">
						...
						<Input id="lName" placeholder="Contoh: Line Produksi A" value={name} onChange={(e) => setName(e.target.value)} className="h-11 bg-white border-gray-200" />
					</div>
					<div className="space-y-1.5">
						<Label htmlFor="lCode" className="text-sm font-semibold text-gray-700">Kode Line</Label>
						<Input id="lCode" placeholder="Contoh: LN-A" value={code} onChange={(e) => setCode(e.target.value)} className="h-11 bg-white border-gray-200" />
					</div>
				</div>
			</ResponsiveModal>
			...
			<ResponsiveModal
				...
				footer={
					<div className="flex gap-3 w-full">
						...
						<Button variant="outline" onClick={() => { setIsEditOpen(false); setSelectedLine(null) }} className="flex-1 h-11 border-gray-200 hover:bg-gray-100 cursor-pointer">Batal</Button>
					</div>
				}
			>
				<div className="space-y-4 py-2">
					<div className="space-y-1.5">
						...
						<Input id="editLName" placeholder="Contoh: Line Produksi A" value={name} onChange={(e) => setName(e.target.value)} className="h-11 bg-white border-gray-200" />
					</div>
					<div className="space-y-1.5">
						<Label htmlFor="editLCode" className="text-sm font-semibold text-gray-700">Kode Line</Label>
						<Input id="editLCode" placeholder="Contoh: LN-A" value={code} onChange={(e) => setCode(e.target.value)} className="h-11 bg-white border-gray-200" />
					</div>
				</div>
			</ResponsiveModal>
			...
			{loading ? (
				...
				<div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
					<div className="h-12 w-12 bg-gray-50 rounded-full flex items-center justify-center mb-4"><Factory className="h-6 w-6 text-gray-300" /></div>
									...
									<tr key={l.id} className="hover:bg-gray-50/40 transition-colors group">
										...
										{isSupervisor && (
											<td className="px-6 py-4 text-center flex items-center justify-center gap-1">
												<Button variant="ghost" size="icon" onClick={() => handleOpenEdit(l)} className="h-8 w-8 text-gray-500 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg cursor-pointer">
													<Edit2 className="h-4 w-4" />
												</Button>
												<Button variant="ghost" size="icon" onClick={() => handleDelete(l.id)} className="h-8 w-8 text-gray-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg cursor-pointer">
