...
export default function MachineTypePage() {
	...
	const loadMachineTypes = async () => {
		try {
			const data = await api.get<any, MachineType[]>("/api/v1/machine-types")
	...
	const handleCreate = async () => {
		...
		try {
			await api.post("/api/v1/machine-types", { name, code })
	...
	const handleUpdate = async () => {
		...
		try {
			await api.put(`/api/v1/machine-types/${selectedMachineType.id}`, { name, code })
	...
	const handleDelete = async (id: string) => {
		...
		try {
			await api.delete(`/api/v1/machine-types/${id}`)
		...
		<div className="space-y-6">
			...
			<div className="space-y-4">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div className="space-y-1">
						<h1 className="text-2xl font-bold text-gray-900 tracking-tight">
							Manajemen Tipe Mesin
						</h1>
						<p className="text-gray-500 text-sm">
							Kelola data kategori dan tipe mesin operasional di pabrik PT. Greenfields Indonesia.
						</p>
					</div>
				...
				<div className="relative w-full">
					...
					<Input
						placeholder="Cari tipe mesin berdasarkan nama atau kode..."
						value={searchQuery}
			...
			{/* Modal for Creating Machine Type */}
			<ResponsiveModal
				open={isFormOpen}
			...
			>
				<div className="space-y-4 py-2">
					<div className="space-y-1.5">
						...
						<Input
							id="mName"
							placeholder="Contoh: Pasteurizer / Homogenizer"
							value={name}
			...
			{/* Modal for Editing Machine Type */}
			<ResponsiveModal
				open={isEditOpen}
