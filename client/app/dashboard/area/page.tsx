...
export default function AreaPage() {
	...
	const loadAreas = async () => {
		try {
			const data = await api.get<any, Area[]>("/api/v1/areas")
			setAreas(data || [])
		} catch (err: any) {
	...
	const handleCreate = async () => {
		...
		try {
			await api.post("/api/v1/areas", { name, code })
	...
	const handleUpdate = async () => {
		...
		try {
			await api.put(`/api/v1/areas/${selectedArea.id}`, { name, code })
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div className="space-y-1">
						<h1 className="text-2xl font-bold text-gray-900 tracking-tight">
							Manajemen Area Produksi
						</h1>
						<p className="text-gray-500 text-sm">
							Kelola data area dan zona penempatan mesin di pabrik PT. Greenfields Indonesia.
						</p>
					</div>
			...
			<ResponsiveModal
				...
				footer={
					<div className="flex gap-3 w-full">
						...
						>
							{submitLoading ? "Menyimpan..." : "Simpan Area"}
						</Button>
						<Button
							variant="outline"
