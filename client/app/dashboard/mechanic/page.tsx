...
export default function MechanicPage() {
	...
	const loadMechanics = async () => {
		try {
			const data = await api.get<any, Mechanic[]>("/api/v1/mechanics")
			setMechanics(data || [])
		} catch (err: any) {
	...
	const handleCreate = async () => {
		...
		try {
			await api.post("/api/v1/mechanics", { name, email, phone, specialization })
	...
	const handleUpdate = async () => {
		...
		try {
			await api.put(`/api/v1/mechanics/${selectedMechanic.id}`, { name, email, phone, specialization })
		...
		<div className="space-y-6">
			...
			<div className="space-y-4">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div className="space-y-1">
						<h1 className="text-2xl font-bold text-gray-900 tracking-tight">
							Manajemen Mekanik & PJ Mesin
						</h1>
						<p className="text-gray-500 text-sm">
							Kelola data mekanik dan penanggung jawab (PIC) operasional mesin di PT. Greenfields Indonesia.
						</p>
					</div>
				...
				<div className="relative w-full">
					...
					<Input
						placeholder="Cari mekanik berdasarkan nama, email, atau spesialisasi..."
						value={searchQuery}
			...
			<ResponsiveModal
				...
				footer={
					<div className="flex gap-3 w-full">
						...
						>
							{submitLoading ? "Menyimpan..." : "Simpan Mekanik"}
						</Button>
						<Button
							variant="outline"
				...
				<div className="space-y-4 py-2">
					...
					<div className="space-y-1.5">
						...
						<Input
							id="mEmail"
							type="email"
							placeholder="Contoh: budi.santoso@greenfields.id"
							value={email}
					...
					<div className="space-y-1.5">
						...
						<Input
							id="mSpec"
							placeholder="Contoh: Electrical, Mechanical, Thermal"
								...
								{filtered.map((m) => (
									<tr key={m.id} className="hover:bg-gray-50/40 transition-colors group">
										<td className="px-6 py-4 font-semibold text-gray-900 text-sm">
											<div className="flex items-center gap-2">
												<div className="h-8 w-8 rounded-full bg-emerald-50 text-emerald-700 flex items-center justify-center font-bold text-xs uppercase">
